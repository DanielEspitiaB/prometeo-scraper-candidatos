...
# ---------------------------------------------------------------------------
# Login (solo para el extractor). Credenciales en secrets, nunca en el código.
# ---------------------------------------------------------------------------
try:
    _user_cfg = st.secrets.get("EXTRACTOR_USER", "")
    _pass_cfg = st.secrets.get("EXTRACTOR_PASS", "")
except Exception:
    _user_cfg = _pass_cfg = ""

if _user_cfg and _pass_cfg and not st.session_state.get("auth_ok"):
    st.subheader("🔐 Acceso")
    with st.form("login_extractor"):
        _u = st.text_input("Usuario")
        _p = st.text_input("Contraseña", type="password")
        if st.form_submit_button("Entrar", type="primary", use_container_width=True):
            if _u == _user_cfg and _p == _pass_cfg:
